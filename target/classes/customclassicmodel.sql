...
select
    c.orderNumber, c.comments, c.customerNumber, c.orderDate, c.requiredDate, c.shippedDate, c.status

from classicmodels.Orders as c order by c.orderNumber;

/*select c.* from cs.Orders as c limit 1;
Delete from cs.Orders;*/

INSERT INTO cs.product
select  c.productLine, c.productCode, c.MSRP, c.buyPrice, c.productDescription, c.productName,c.productScale,
        c.productVendor, c.quantityInStock from classicmodels.products as c ;

/*select c.* from cs.product as c limit 1;
...
/*select c.* from cs.orderdetail as c limit 1;
Delete from cs.orderdetail;*/
...
#CREATE DATABASE /*!32312 IF NOT EXISTS*/`customclassicmodels` /*!40100 DEFAULT CHARACTER SET latin1 */;

USE `customclassicmodels`;
...
select * from  customers; /* remove extention */
...
select * from orderdetails; /* see it again but comment can be gone*/
ALTER TABLE table_name DROP [COLUMN] column_name;

select * from orders; /* see check numbers*/
ALTER TABLE table_name DROP [COLUMN] column_name;

select * from  payments;

select * from  productlines ;  /* how to use image and html in database */
ALTER TABLE table_name DROP [COLUMN] column_name;
select * from  products;/* product scale, prodDescrioption, MSRP*/
...
/*[Section I] Non Dependent talbes */
...
# Add constraints  for productlines table #
ALTER TABLE productlines ADD constraint  pk_productlines PRIMARY KEY  (productLine);
#Drop Constraints for productlines table #
ALTER TABLE productlines DROP KEY  `PRIMARY`;
...
# Add constraints  for customers table #
Alter table offices add constraint pk_officeCode Primary key (officeCode);
# Drop constraints  for customers table #
Alter table offices drop constraint `Primary`;
insert  into `offices`(`officeCode`,`city`,`phone`,`addressLine1`,`addressLine2`,`state`,`country`,`postalCode`,`territory`)
values ('1','San Francisco','+1 650 219 4782','100 Market Street','Suite 300','CA','USA','94080','NA');

/* [Section II]  Dependent Dependent talbes */

/* (3) Table ==> `employees depends on employees and offices*/
USE `customclassicmodels`;
...
ALTER TABLE employees ADD constraint  pk_employeeNubmer PRIMARY KEY  (employeeNumber);
ALTER TABLE employees ADD CONSTRAINT reportsTo FOREIGN KEY  (reportsTo) REFERENCES employees (employeeNumber);
ALTER TABLE employees ADD CONSTRAINT officeCode FOREIGN KEY  (officeCode) REFERENCES offices (officeCode);
#Drop Constraints for customers #
ALTER TABLE employees DROP constraint reportsTo;
ALTER TABLE employees DROP constraint officeCode;
ALTER TABLE employees Drop Index reportsTo;
ALTER TABLE employees Drop Index officeCode;
ALTER TABLE employees DROP KEY  `PRIMARY`;
insert  into `employees`(`employeeNumber`,`lastName`,`firstName`,`extension`,`email`,`officeCode`,`reportsTo`,`jobTitle`)
values (1002,'Murphy','Diane','x5800','dmurphy@classicmodelcars.com','1',NULL,'President');

/*(4) ==>`customers depend on Employees */
USE `customclassicmodels`;
...
# Add constraints  for customers table  Depends on employees table #
CREATE INDEX  salesRepEmployeeNumber ON customers (salesRepEmployeeNumber) ;
ALTER TABLE customers ADD CONSTRAINT  pk_customerNumber  PRIMARY KEY (customerNumber);
ALTER TABLE customers ADD CONSTRAINT salesRepEmployeeNumber FOREIGN KEY  (salesRepEmployeeNumber) REFERENCES employees (employeeNumber);
#Drop Constraints for customers #
ALTER TABLE customers DROP constraint salesRepEmployeeNumber;
ALTER TABLE customers Drop Index salesRepEmployeeNumber;
ALTER TABLE customers DROP KEY  `PRIMARY`;
#Sample Insert values for customers table #
INSERT INTO `customers`(`customerNumber`,`customerName`,`contactLastName`,`contactFirstName`,`phone`,`addressLine1`,`addressLine2`,`city`,`state`,`postalCode`,`country`,`salesRepEmployeeNumber`,`creditLimit`)
VALUE(103,'Atelier graphique','Schmitt','Carine ','40.32.2555','54, rue Royale',NULL,'Nantes',NULL,'44000','France',1370,'21000.00');


/*(5)==> `orders` depend on customers*/
USE `customclassicmodels`;
...
CREATE INDEX  orderNumber ON orderdetails (orderNumber) ;
ALTER TABLE orders ADD constraint  pk_orderNumber PRIMARY KEY  (orderNumber);
ALTER TABLE orders ADD CONSTRAINT customerNumber FOREIGN KEY  (customerNumber) REFERENCES customers (customerNumber);
#Drop Constraints for orders table #
ALTER TABLE orders DROP INDEX  orderNumber;
ALTER TABLE orders DROP constraint customerNumber;
ALTER TABLE orders DROP KEY  `PRIMARY`;
/*Data for the table `orders` */
insert  into `orders`(`orderNumber`,`orderDate`,`requiredDate`,`shippedDate`,`status`,`comments`,`customerNumber`)
values (10100,'2003-01-06','2003-01-13','2003-01-10','Shipped',NULL,363);


/*(6)==>`payments` depends on customers*/
USE `customclassicmodels`;
...
CREATE TABLE `payments` (
  ...
  String paymentDate` date NOT NULL,
  String amount` decimal(10,2) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
# Add constraints  for payments table #
CREATE INDEX payments ON payments (customerNumber);
ALTER TABLE payments ADD constraint  pk_payments PRIMARY KEY  (customerNumber,checkNumber);
ALTER TABLE payments ADD CONSTRAINT customerNumber FOREIGN KEY  (customerNumber) REFERENCES customers (customerNumber);
#Drop Constraints for payments table #
ALTER TABLE payments Drop Index customerNumber;
ALTER TABLE payments Drop Index customerNumber;
ALTER TABLE payments DROP KEY  `PRIMARY`;  #mariadb creae the pk with PRIMARY name weird
...
CREATE TABLE
  ...
  String productCode;// varchar(15) NOT NULL,
  ...
  String quantityInStock;// smallint(6) NOT NULL,
  String buyPrice;// decimal(10,2) NOT NULL,
  String MSRP;// decimal(10,2) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
# Add constraints  for orders table #
CREATE INDEX  productLine ON products (productLine) ;
ALTER TABLE products ADD constraint  pk_productCode PRIMARY KEY  (productCode);
...
CREATE TABLE `orderdetails` (
  String orderNumber;//  int(11) NOT NULL,
  String productCode; // varchar(15) NOT NULL,
  String quantityOrdered ;// int(11) NOT NULL,
...
# Add constraints  for orderdetails table #
CREATE INDEX  orderNumber ON orderdetails (orderNumber) ;
CREATE INDEX  productCode ON orderdetails (productCode) ;
ALTER TABLE orderdetails ADD constraint  cpk_orderNumberproductCode PRIMARY KEY  (orderNumber,productCode);
ALTER TABLE orderdetails ADD CONSTRAINT orderNumber FOREIGN KEY  (orderNumber) REFERENCES orders (orderNumber);
ALTER TABLE orderdetails ADD CONSTRAINT productCode FOREIGN KEY  (productCode) REFERENCES products (productCode);
#Drop Constraints for orderdetails table #
ALTER TABLE employees DROP constraint orderNumber;
ALTER TABLE employees DROP constraint productCode;
ALTER TABLE employees Drop Index orderNumber;
ALTER TABLE employees Drop Index productCode;
ALTER TABLE employees DROP KEY  `PRIMARY`;
...
values (10100,'S18_1749',30,'136.00',3);
 ...
 # SECTION II -  Data for all tables
